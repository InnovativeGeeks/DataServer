<!DOCTYPE html>
<html>

<head>
    <title>sin function stream</title>
    <!--<link rel="stylesheet" type="text/css" href="../css/sinFunction.css">-->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <script type="text/javascript" src="../js/sinFunction.js"></script>
</head>

<body>
    <form class="form-container" onsubmit="return validate()">
        <p id="error"></p>
        <h3>f(x) = a sin (bx+c) + d</h3>
        <h4>please enter the values a,b,c,d and x to generate the function value</h4>
        <div class="form-grp">
            <label for="a"> a(amplitude)</label><br>
            <input type="number" step="any" name="a" placeholder="a(amplitude)" id="a">
        </div>
        <div class="form-grp">
            <label for="b"> b(horizontal dilation)</label><br>
            <input type="number" step="any" name="b" placeholder="b(horizontal dilation)" id="b">
        </div>
        <div class="form-grp">
            <label for="c"> c(phase shift)</label><br>
            <input type="number" step="any" name="c" placeholder="c(phase shift)" id="c">
        </div>
        <div class="form-grp">
            <label for="d"> d(vertical translation)</label><br>
            <input type="number" step="any" name="d" placeholder="d(vertical translation)" id="d">
        </div>
        <div class="form-grp">
            <label for="d"> x(increment value)</label><br>
            <input type="number" step="any" name="d" placeholder="x(increment value)" id="i">
        </div>
        <div class="form-grp">
            <!--<input type="submit" name="calulcate-button" value="calculate">-->
            <label for="dtn"> dataname</label><br>
            <input type="input" name="dtn" placeholder="dataname" id="dtn">
            <br>
            <label for="fn"> sourcename</label><br>
            <input type="input" name="fn" placeholder="filename" id="fn">
        </div>
    </form>
</body>
</html>