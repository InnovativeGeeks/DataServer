...
function validate() {
    ...
    for (i = 1; i < streams + 1; i++) {
        ...
        if (dt == 'text') {
            var value = document.getElementById('inputValue_' + i).value.trim();
            if (value == '') {
                alert('all value fields are required for stream ' + i);
                res = false;
            }
        } else if (dt == 'numeric') {
            var num = document.getElementById('inputFunction_' + i).value;
